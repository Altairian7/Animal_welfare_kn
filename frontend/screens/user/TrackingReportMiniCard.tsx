...
const TrackingReportMiniCard: React.FC<TrackingReportMiniCardProps> = ({ 
  ...
  const getImageSource = () => {
    if (!report.image_url || imageError || showFallback) {
      return null;
    }
  ...
  const getHealthPreview = () => {
    if (report.injury_summary) return report.injury_summary;
    if (report.symptoms && report.symptoms.length > 0)
      return report.symptoms.slice(0, 2).join(", ");
      ...
      >
        ...
        <View style={{ flexDirection: "row", padding: 12, paddingBottom: 12 }}>
          ...
          {getImageSource() && !showFallback ? (
            <Image
              ...
              resizeMode="cover"
              onError={() => {
                console.error("Tracking mini card image error");
                setImageError(true);
          ...
          <View style={{ flex: 1, paddingBottom: 8 }}>
            ...
            <Text
              ...
              numberOfLines={1}
            >
              {safeText(report.species)} • {safeText(report.age)} •{" "}
            ...
            >
              <Chip
                ...
                compact
                style={{
                  backgroundColor: addAlpha(getStatusColor(report.status), 0.2),
                  height: 30,
                }}
                textStyle={{
        ...
        >
          <View style={{ alignItems: "center" }}>
            ...
            >
              {report.status === "in_progress" ? "Active" : safeText(report.status)}
            </Text>
          </View>
          <View style={{ alignItems: "center" }}>
            <Text
          ...
          <View style={{ alignItems: "center" }}>
            ...
            <View style={{ flexDirection: "row", alignItems: "center" }}>
              <Ionicons
                name="analytics"
                size={10}
                color={theme.colors.tertiary || "#9C27B0"}
              />
              <Text
                style={{
                  fontSize: 10,
              ...
              >
                {report.status === "in_progress" ? "Tracking" : "Waiting"}
              </Text>
            </View>
          </View>
        </View>
